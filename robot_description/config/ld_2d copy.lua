...
options = {
  ...
  tracking_frame = "imu_link",  -- 跟踪帧的名称
  ...
  use_pose_extrapolator = true,  -- 提升轨迹连续性
  ...
  num_subdivisions_per_laser_scan = 2,  -- 每个激光扫描的细分数量
  num_point_clouds = 0,  -- 点云的数量
  lookup_transform_timeout_sec = 0.2,  -- 查找变换的超时时间（秒）
  submap_publish_period_sec = 0.3,  -- 子地图发布周期（秒）
  pose_publish_period_sec = 10e-3,  -- 姿态发布周期（秒）
  trajectory_publish_period_sec = 50e-3,  -- 轨迹发布周期（秒）
...
TRAJECTORY_BUILDER_2D.min_range = 0.2  -- 限制在雷达最小扫描范围，比机器人半径小的都忽略
TRAJECTORY_BUILDER_2D.max_range = 10  -- 限制在雷达最大扫描范围
...
TRAJECTORY_BUILDER_2D.imu_gravity_time_constant = 15.0  -- 重力加速度时间常数估计
...
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.8  --0.6改成0.7,全局定位最小分数，低于此分数则认为目前全局定位不准确
