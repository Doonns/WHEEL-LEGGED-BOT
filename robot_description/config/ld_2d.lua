...
options = {
  ...
  use_pose_extrapolator = false,  -- 提升轨迹连续性
  ...
  pose_publish_period_sec = 0.1,  -- 姿态发布周期（秒）
  trajectory_publish_period_sec = 0.1,  -- 轨迹发布周期（秒）   0.005
  rangefinder_sampling_ratio = .7,  -- 测距仪采样比率
  odometry_sampling_ratio = 1.,  -- 里程计采样比率
  fixed_frame_pose_sampling_ratio = 1.,  -- 固定帧姿态采样比率
  imu_sampling_ratio = .3,  -- IMU采样比率
  landmarks_sampling_ratio = 1.,  -- 地标采样比率
}
 
MAP_BUILDER.use_trajectory_builder_2d = true  -- 是否启动2D SLAM
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 30  -- 2D轨迹构建器中子地图的范围数据数量
